...
const WaveSimulation = ({ width, height }) => {
  const canvas = useRef();
  ...
  useEffect(() => {
    ...
    function generatePointVertex(time) {
        ...
        for (let pointX = -50; pointX < X; pointX++) {
          ...
          for (let i = 0; i < strip.length; i++) {
            const smallNoiseVal = noise3d(pointX * 0.2*2, i / (X / 2), (time + 1) / 100) / 6;
            const bigNoiseVal = Math.abs(noise3d(pointX * 0.05*2, i / (X / 0.5), (time + 1) / 50) / 4);
            const massiveNoiseVal = Math.abs(noise3d(pointX * 0.001, i / (X / 0.05), ((time/2) + 1) / 200));
            const sinWave = Math.sin(((time) / 20) + pointX*1.25 * 0.07)
            const enhancedSinWave = Math.min((sinWave * 1), 0.90);
            const noiseSin = createWave();
            function createWave() {
              if (smallNoiseVal + sinWave <= -0.9) {
                return enhancedSinWave + (smallNoiseVal/2) + (bigNoiseVal) + (massiveNoiseVal*2);
              } else {
                return enhancedSinWave + (smallNoiseVal/8) + (bigNoiseVal) + (massiveNoiseVal*2);
              }
            }
    ...
    gl.shaderSource(
      ...
      void main() {
          vColor = color;
          vPosition = position + (color * vec3(0.01, 0.1, 0.19));
    ...
    gl.shaderSource(
      ...
      void main() {
          gl_FragColor = vec4(0.3 - vColor.r, 0.3 - vColor.g, 1.5 - vColor.b, 1.0);
      }
    `
    );
    ...
    mat4.translate(viewMatrix, viewMatrix, [0, -0.125, 3.0]);
    mat4.rotateX(modelMatrix, modelMatrix, Math.PI / 1.5);
    mat4.rotateZ(modelMatrix, modelMatrix, Math.PI / 1.5);
